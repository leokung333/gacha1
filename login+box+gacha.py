...
def sanitize_filename(filename):
    ...
    # เก็บเลขในวงเล็บ (ถ้ามี)
    number_match = re.search(r'\((\d+)\)', name)
    number = f"({number_match.group(1)})" if number_match else ""
    
    # ทำความสะอาดชื่อไฟล์แต่เก็บภาษาไทยไว้
...
def check_for_hero1(device):
    """ฟังก์ชันตรวจสอบการมีอยู่ของ heroo2.png"""
    try:
        print(f"กำลังค้นหา heroo2.png บนอุปกรณ์ {device.serial}")
        cap = device.screencap()
        image = np.frombuffer(cap, dtype=np.uint8)
        adb_img = cv2.imdecode(image, cv2.IMREAD_COLOR)
        
        hero_pos = ImgSearchADB(adb_img, 'img/heroo2.png')
        if hero_pos:
            print(f"พบ heroo2.png บนอุปกรณ์ {device.serial}")
            return True
        return False
    except Exception as e:
        print(f"เกิดข้อผิดพลาดในการค้นหา heroo2.png: {e}")
        return False
...
def process_swap_shop(device):
    ...
    while running:
        try:
            ...
            # ตรวจสอบ hero images เป็นอันดับแรก
            if check_hero_images(adb_img):
                print(f"\nพบ hero image บนอุปกรณ์ {device.serial} - ดำเนินการ backup ทันที")
            ...
            # ตรวจสอบ stopgacha5, stopgacha7, และ stopgacha8
            for stop_img in ['stopgacha5.png', 'stopgacha7.png', 'stopgacha8.png']:
                stop_pos = ImgSearchADB(adb_img, f'img/{stop_img}')
                if stop_pos:
                    print(f"\nพบ {stop_img} บนอุปกรณ์ {device.serial} - จบการทำงาน")
                    # ถ้าไม่พบ hero images ทำการ backup ไปที่ random-Fail
                    if not check_hero_images(adb_img):
                        print(f"ไม่พบ hero images บนอุปกรณ์ {device.serial} - ทำการ backup ไปที่ random-Fail")
                        if backup_failed_game_data(device):
                            print(f"Backup ไปที่ random-Fail สำเร็จสำหรับอุปกรณ์ {device.serial}")
                        else:
                            print(f"Backup ไปที่ random-Fail ล้มเหลวสำหรับอุปกรณ์ {device.serial}")
            ...
            # ตรวจสอบ waitgacha.png และเลือกช่อง
            ...
            if time.time() % 30 < 1:
                print(f"\nStatus Update - Device {device.serial}")
...
# เพิ่มคลาส NetworkMonitor เพื่อจัดการการตรวจสอบ fixnet.png
class NetworkMonitor:
    def __init__(self):
        self.last_check = time.time()
        self.check_interval = 10  # ตรวจสอบทุก 10 วินาที
...
# เพิ่มฟังก์ชันใหม่สำหรับการจัดการการเชื่อมต่อ
def handle_network_issues(device):
    """ฟังก์ชันสำหรับจัดการปัญหาการเชื่อมต่อ"""
    try:
        # พยายามแก้ไขปัญหาการเชื่อมต่อ
        cap = device.screencap()
        image = np.frombuffer(cap, dtype=np.uint8)
        adb_img = cv2.imdecode(image, cv2.IMREAD_COLOR)
        
        # ลำดับการตรวจสอบและแก้ไขปัญหา
        images_to_check = [
            ('fixnet.png', 'ปัญหาการเชื่อมต่อ')
        ]
        
        for img_name, description in images_to_check:
            pos = ImgSearchADB(adb_img, f'img/{img_name}')
            if pos:
                print(f"พบ {description} บนอุปกรณ์ {device.serial}")
                device.shell(f"input tap {pos[0][0]} {pos[0][1]}")
                time.sleep(1)
                return True
        ...
        print(f"เกิดข้อผิดพลาดในการจัดการปัญหาการเชื่อมต่อ: {e}")
        return False


# ปรับปรุงฟังก์ชัน adaptive_delay
def adaptive_delay(device, base_delay=1):
    """ฟังก์ชันหน่วงเวลาแบบปรับได้ พร้อมตรวจสอบการเชื่อมต่อ"""
    network_monitor = NetworkMonitor()
    try:
        cap = device.screencap()
        image = np.frombuffer(cap, dtype=np.uint8)
        ...
        # ตรวจสอบการเชื่อมต่อระหว่างการหน่วงเวลา
        if network_monitor.check_network(device, adb_img):
            base_delay = base_delay * 1.5  # เพิ่มเวลาหน่วงถ้าพบปัญหาการเชื่อมต่อ
            
        cpu_info = device.shell("cat /proc/stat")
        if "cpu" in cpu_info.lower():
...
def main_box(device):
    ...
    try:
        ...
        box_settings = config.get('box_settings', {
            'first_round': 1,   # ค่าเริ่มต้นเปิดใช้งานรอบแรก
            'second_round': 1    # ค่าเริ่มต้นเปิดใช้งานรอบที่สอง
        })
    except Exception as e:
...
def process_single_file_for_device(device):
    ...
    try:
        ...
        # สร้างชื่อไฟล์ที่ปลอดภัยแต่ยังคงภาษาไทย
        safe_xml_file = sanitize_filename(xml_file)
        
        # สร้าง Path objects สำหรับจัดการพาธ
        ...
        backup_folder = Path(source_folder) / "backup_thai"
        ...
        try:
            ...
            # สำรองไฟล์ต้นฉบับ
            backup_path = backup_folder / f"{safe_xml_file}"
            try:
                import shutil
                shutil.copy2(str(original_file_path), str(backup_path))
                print(f"สำรองไฟล์ไว้ที่: {backup_path}")
            except Exception as e:
                print(f"ไม่สามารถสำรองไฟล์: {str(e)}")

            # คัดลอกไฟล์โดยใช้ binary mode
            max_retries = 3
            success = False
            ...
            # ส่งไฟล์โดยใช้ adb push command
            for attempt in range(max_retries):
                try:
                    # สร้าง command ที่รองรับ Unicode
                    push_command = [
                        'adb',
                        '-s', device.serial,
                        'push',
...
def backup_failed_game_data(device):
    ...
    try:
        ...
        print(f"\nDevice {device.serial}: === Starting Failed Backup Process ===")
        ...
        if result.returncode == 0 and os.path.exists(backup_path):
            ...
            print(f"\nDevice {device.serial}: === Failed Backup Successful ===")
...
def process_single_device(device):
    ...
    """
    def load_feature_config(feature_name, default=0):
        """Load feature configuration from config file"""
        try:
            with open('config.json', 'r', encoding='utf-8') as f:
                config = json.load(f)
            return config.get(feature_name, default)
        except Exception as e:
            print(f"Device {device.serial}: ไม่สามารถโหลดการตั้งค่า {feature_name}: {e}")
            return default
    ...
    def log_status(message, status_type="INFO"):
        ...
        current_time = "2025-05-25 05:57:17"  # UTC time
    ...
    while True:
        try:
            ...
            seven_day_enabled = load_feature_config('7day', 0)
            swap_shop_enabled = load_feature_config('swap_shop', 1)  # default เป็น 1

            # แสดงสถานะการตั้งค่า
            log_status(f"""
            === การตั้งค่าการทำงาน ===
            7 Day: {'เปิด' if seven_day_enabled else 'ปิด'}
            Swap Shop: {'เปิด' if swap_shop_enabled else 'ปิด'}
            =========================
            """)
            ...
            if process_single_file_for_device(device):
                ...
                if login_result == "restart":
                    log_status("พบ fixid ครบ 8 รอบ กำลังทำ backup ไปที่ login-fail")
                    # เพิ่มการ backup ไปที่โฟลเดอร์ login-fail
                    if backup_failed_login(device):
                        log_status("Backup ไปที่ login-fail สำเร็จ")
                    else:
                        log_status("Backup ไปที่ login-fail ล้มเหลว", "ERROR")
                    ...
                    # ตรวจสอบและทำงาน 7day
                    if seven_day_enabled:
                        log_status("เริ่มกระบวนการ 7 วัน")
                        func_7day(device)
                        adaptive_delay(0.8)
                    else:
                        log_status("ข้ามกระบวนการ 7 วัน (ปิดใช้งาน)")
                    ...
                    # ตรวจสอบและทำงาน swap_shop
                    if swap_shop_enabled:
                        log_status("เริ่มกระบวนการ swap shop")
                        swap_result = process_swap_shop(device)
                        
                        # ตรวจสอบผลลัพธ์จาก swap_shop
                        if swap_result == "backup_complete":
                            log_status("Swap shop และ backup เสร็จสมบูรณ์")
                        elif swap_result == "complete":
                            log_status("Swap shop เสร็จสมบูรณ์")
                    else:
                        log_status("ข้ามกระบวนการ swap shop (ปิดใช้งาน)")
...
def backup_failed_login(device):
    """
    Backup game data when fixid count reaches 8 to login-fail folder
    """
    try:
        # สร้างโฟลเดอร์ login-fail ถ้ายังไม่มี
        current_dir = os.path.dirname(os.path.abspath(__file__))
        ...
        print(f"\nDevice {device.serial}: === Starting Login-Fail Backup Process ===")
        print(f"Current Date and Time (UTC): 2025-05-25 05:55:54")
        print(f"Current User's Login: tablegub")
        print(f"Backup path: {backup_path}")
        ...
        if result.returncode == 0 and os.path.exists(backup_path):
            file_size = os.path.getsize(backup_path)
            print(f"\nDevice {device.serial}: === Login-Fail Backup Successful ===")
            print(f"Backup location: {backup_path}")
            print(f"File size: {file_size} bytes")
            return True
        else:
            print(f"\nDevice {device.serial}: Login-Fail Backup failed: {result.stderr}")
            return False
...
def Main():
    ...
    # สร้าง NetworkMonitor instance สำหรับใช้ทั้งโปรแกรม
    network_monitor = NetworkMonitor()
    ...
    while True:
        try:
            ...
            # สร้างฟังก์ชันสำหรับตรวจสอบ fixnet.png
            def check_fixnet_for_device(device):
                while not stop_event.is_set():
                    try:
                        ...
                        fixnet_pos = ImgSearchADB(adb_img, 'img/fixnet.png')
                        if fixnet_pos:
                            print(f"\nพบปัญหาการเชื่อมต่อ (fixnet.png) บนอุปกรณ์ {device.serial}")
                            device.shell(f"input tap {fixnet_pos[0][0]} {fixnet_pos[0][1]}")
                            time.sleep(1)
                    except Exception as e:
                        print(f"เกิดข้อผิดพลาดในการตรวจสอบ fixnet.png บนอุปกรณ์ {device.serial}: {e}")
                    time.sleep(2)  # รอ 2 วินาทีก่อนตรวจสอบครั้งถัดไป
            
            threads = []
            network_threads = []
            
            # สร้าง thread สำหรับตรวจสอบ fixnet.png แยกต่างหากสำหรับแต่ละอุปกรณ์
            for device in devices:
                # Thread สำหรับการทำงานหลัก
                main_thread = Thread(target=process_single_device, args=(device,))
                main_thread.daemon = True
                main_thread.start()
                threads.append(main_thread)
                
                # Thread สำหรับตรวจสอบ fixnet.png
                network_thread = Thread(target=check_fixnet_for_device, args=(device,))
                network_thread.daemon = True
                network_thread.start()
                network_threads.append(network_thread)
            
            try:
                # รอให้ thread หลักทำงานเสร็จ
                for thread in threads:
                    thread.join()
            except KeyboardInterrupt:
                print("\nได้รับสัญญาณหยุดการทำงาน กำลังปิดโปรแกรม...")
                stop_event.set()  # ส่งสัญญาณให้ thread ตรวจสอบ fixnet หยุดทำงาน
                
                # รอให้ thread ตรวจสอบ fixnet หยุดทำงาน
                for thread in network_threads:
                    thread.join(timeout=2)  # รอสูงสุด 2 วินาที
                break
            except Exception as e:
